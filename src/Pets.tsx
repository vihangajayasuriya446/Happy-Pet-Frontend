...
const Pets: React.FC = () => {
  ...
  const { data: pets = [] as Pet[] } = useQuery<Pet[], Error>({
    queryKey: ["pets"],
    queryFn: async () => {
      const response = await axios.get("http://51.21.197.93:8080/api/v1/gets");
  ...
  const addPetMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await axios.post("http://51.21.197.93:8080/api/v1/addpet", data, {
  ...
  const updatePetMutation = useMutation({
    mutationFn: async (data: FormData) => {
      const response = await axios.put("http://51.21.197.93:8080/api/v1/updatepet", data, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      return response.data;
    },
  ...
  const deletePetMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await axios.delete(`http://51.21.197.93:8080/api/v1/deletepet/${id}`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["pets"] });
