...
const OwnerTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOwners = async () => {
      setIsLoading(true);
      try {
        const response = await fetch('http://13.60.206.42:8080/api/v1/getowners', {
          headers: {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('http://13.60.206.42:8080/api/v1/getusers');
        if (!response.ok) {
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`http://13.60.206.42:8080/api/v1/deleteowner/${id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleToggle = async (id: number, currentConfirmation: string) => {
    ...
    try {
      const response = await fetch(`http://13.60.206.42:8080/api/v1/updateowner/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `Bearer ${token}`,
