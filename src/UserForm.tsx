...
const OwnerForm: React.FC = () => {
  const { petId } = useParams<{ petId: string }>(); // Get petId from route
  const [owner, setOwner] = useState({ 
    ownerName: '', 
    address: '', 
    contactNumber: '', 
    petId: petId ? parseInt(petId, 10) : 0, // Set petId if available
    confirmation: 'No', // Default confirmation to "No"
  });
